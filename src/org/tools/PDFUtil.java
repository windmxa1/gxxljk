...
public class PDFUtil {
	 ...
	 * 创建PDF文件
	 */
	private static boolean createAlarmPDF(File file, List<VAlarmId> list) {
		...
		try {
			...
			PdfPCell cell = new PdfPCell(new Paragraph("报警信息记录", headerFont));
			cell.setHorizontalAlignment(1);
			cell.setColspan(7);
			table.addCell(cell);
			table.addCell(selector.process("记录id"));
			table.addCell(selector.process("发生时间"));
			table.addCell(selector.process("距离"));
			table.addCell(selector.process("描述"));
			table.addCell(selector.process("等级"));
			table.addCell(selector.process("确认"));
			table.addCell(selector.process("持续时间"));

			for (VAlarmId a : list) {
				table.addCell(selector.process("" + a.getId()));
				table.addCell(selector.process(a.getVtime()));
				table.addCell(selector.process(a.getDistance()));
				table.addCell(selector.process(a.getDescription()));
				table.addCell(selector.process(a.getLevel()));
				table.addCell(selector.process("" + a.getAck()));
				table.addCell(selector.process(a.getLastTime()));
			}
	 ...
	 * 创建PDF文件
	 */
	private static boolean createExceptionPDF(File file, List<VExceptionId> list) {
		System.out.println("生成pdf");
		Rectangle rect = new Rectangle(PageSize.A4);
		Document doc = new Document(rect, 50.0F, 50.0F, 50.0F, 50.0F);
		try {
			...
			PdfPCell cell = new PdfPCell(new Paragraph("异常信息记录", headerFont));
			cell.setHorizontalAlignment(1);
			cell.setColspan(4);
			table.addCell(cell);
			table.addCell(selector.process("记录id"));
			...
			for (VExceptionId e : list) {
				table.addCell(selector.process("" + e.getId()));
				table.addCell(selector.process(e.getVtime()));
				table.addCell(selector.process(e.getDescription()));
				table.addCell(selector.process("" + e.getAck()));
			}
			doc.add(table);
			doc.close();
			return true;
	...
	private static boolean createLogPDF(File file, List<VLogId> list) {
		...
		try {
			...
			table.addCell(selector.process("操作数据id"));
			...
			for (VLogId l : list) {
				...
				table.addCell(selector.process("" + l.getDataId()));
